...
if __name__ == "__main__":
    ...
    for i, (train_index, test_index) in enumerate(kf):
        ...
        scores_val = np.exp(clf.predict(d_valid, ntree_limit=clf.best_ntree_limit)) - shift
        blend_train[test_index] = scores_val
        cv_score = mean_absolute_error(np.exp(y_val)-shift, scores_val)
        val_loss.append(cv_score)
        print('eval-MAE: %.6f' % cv_score)
        y_pred = np.exp(clf.predict(d_test, ntree_limit=clf.best_ntree_limit)) - shift
        predictions.append(y_pred)
    
    score = sum(val_loss) / n_folds
